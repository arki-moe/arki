...
TOOLS['delete_text'] = new Tool({
  name: 'delete_text',
  parameters: {
    path: { type: 'string', description: 'File path' },
    target: { type: 'string', description: 'Target string to delete (must be unique)' },
  },
  ...
  execute: async (args, context) => {
    ...
    try {
      ...
      await cachedFileSystem.delete(fullPath, target, context.agentId);
      return `Deleted target from ${filePath}. Use flush_changes to write to disk.`;
    } catch (error) {
      ...
      return {
        content: `Failed to delete text: ${error instanceof Error ? error.message : String(error)}`,
        isError: true,
      };
    }
  },
